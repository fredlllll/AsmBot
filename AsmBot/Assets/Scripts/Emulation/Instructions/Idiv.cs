...
{
    [InstructionInfo(MnemonicOpCode.IDIV)]
    public class Idiv : Instruction
    {
        public Idiv(Operand[] operands) : base(operands) { }
...
FI;
        */
        ...
        {
            var dividend = (short)cpu.registers.GetRegisterWord(Register.R16_AX);
            var divisor = (sbyte)opDivisor.GetByte(cpu);
            if (divisor == 0)
            {
        ...
        {
            int dividend = (cpu.registers.GetRegisterWord(Register.R16_DX) << 16) | cpu.registers.GetRegisterWord(Register.R16_AX);
            var divisor = (short)opDivisor.GetWord(cpu);
            if (divisor == 0)
            {
                throw new DivideByZeroException();
            }
