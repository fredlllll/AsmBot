...
{
    [InstructionInfo(MnemonicOpCode.CMP)]
    public class Cmp : Instruction
    {
        public Cmp(Operand[] operands) : base(operands) { }

        //same as SUB, without affecting the source/destination operands

        //sets OF,CF,ZF,SF

        private void Execute8Bit(CPU cpu, Operand destination, Operand source)
        {
            var b1 = (sbyte)destination.GetByte(cpu);
            var b2 = (sbyte)source.GetByte(cpu);
            var result = b1 - b2;
        ...
        {
            var w1 = (short)destination.GetWord(cpu);
            var w2 = (short)source.GetWord(cpu);
            var result = w1 - w2;
