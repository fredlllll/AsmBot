...
{
    [InstructionInfo(MnemonicOpCode.SHL)]
    public class Shl : Instruction
    {
        public Shl(Operand[] operands) : base(operands) { }
        ...
        public override void Execute(CPU cpu)
        {
            ...
            switch (destination.GetSize())
            {
                case OperandSize._8Bits:
                    destination.SetByte(cpu, (byte)(destination.GetByte(cpu) << bits));
                    break;
                case OperandSize._16Bits:
                    destination.SetWord(cpu, (ushort)(destination.GetWord(cpu) << bits));
                    break;
                default:
